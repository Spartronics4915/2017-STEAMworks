package com.spartronics4915.frc2017;

public class RobotMap
{
    // Robot dimensions
    public static final int ROBOT_LENGTH = 35;
    public static final int ROBOT_WIDTH = 38;

    // CAN bus ids  (all SRX devices) --------------------------------
    public static final int NUM_CAN_DEVICES = 9; // includes PDP

    public static final int DRIVE_TRAIN_MOTOR_PORT_FOLLOWER = 1;
    public static final int DRIVE_TRAIN_MOTOR_PORT_MASTER = 3;

    public static final int DRIVE_TRAIN_MOTOR_STARBOARD_FOLLOWER = 2;
    public static final int DRIVE_TRAIN_MOTOR_STARBOARD_MASTER = 4;

    public static final int INTAKE_MOTOR = 5;

    public static final int LAUNCHER_MOTOR = 7;
    public static final int AGITATOR_MOTOR = 6;

    public static final int CLIMBER_MOTOR = 8;

}
